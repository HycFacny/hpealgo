...
class CrossResolutionWeighting(nn.Module):
    def __init__(self, channels, ratio=16, activations=['relu', 'sigmoid'], norm='bn'):
        ...
        self.norm1 = _get_norm_layer(norm, sum_channels // ratio)
        self.activation1 = _get_activation_fn(activations[0])
        self.conv2 = nn.Conv2d(
            sum_channels // ratio,
            sum_channels,
        ...
        self.norm2 = _get_norm_layer(norm, sum_channels)
        self.activation2 = _get_activation_fn(activations[1])        

    def forward(self, x):
        min_size = x[-1].size()[-2:]
        out = [F.adaptiev_avg_pool2d(s, min_size) for s in x[:-1]] + [x[-1]]
...
class Stem(nn.Module):
    def __init__(self, in_channels, stem_channels, out_channels, expand_ratio, norm='bn'):
        ...
        self.conv1 = nn.Sequential(
            ...
            _get_norm_layer(norm, stem_channels),
        ...
        self.branch1 = nn.Sequential(
            ...
            _get_norm_layer(norm, branch_channels),
            nn.ReLU(inplace=True),
        ...
        for i in range(num_branches):
            if i != num_branches - 1:
                projects.append(nn.Sequential(
                    nn.Conv2d(self.in_channels[i], self.in_channels[i], kernel_size=3, stride=1, padding=1, groups=in_channels),
            ...
            for pre in range(self.num_branches):
                ...
                if pre > post:
                    layer.append(nn.Sequential(
                        ...
                        _get_norm_layer(self.norm, out_channel),
                    ...
                    for cur in range(post - pre):
                        if cur == post - pre - 1:
                            conv3x3s.append(nn.Sequential(
                                ...
                                _get_norm_layer(self.norm, in_channel),
                                nn.Conv2d(in_channel, out_channel, kernel_size=1, stride=1, padding=0, bias=False),
                                _get_norm_layer(self.norm, out_channel)
                            ))
                        else:
            ...
            if post < num_branches_pre:
                if self.in_channels[post] != self.out_channels[post]:
                    transition_layers.append(nn.Sequential(
                        # conv 3, 1, 1, False
                        nn.Conv2d(self.in_channels[post], self.in_channels[post], kernel_size=3, stride=1, padding=1, group=self.in_channels[post], bias=False),
                        _get_norm_layer(self.in_channels[post]),
                        nn.Conv2d(self.in_channels[post], self.out_channels[post], kernel_size=3, stride=1, padding=0, bias=False),
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    if is_train and cfg.MODEL.INIT_WEIGHT:
